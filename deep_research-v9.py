...
class DeepResearchTool:
    ...
    def __init__(self, llm_provider: str = "anthropic", cache_dir: str = ".research_cache"):
        ...
        if llm_provider == "anthropic":
        ...
        elif llm_provider == "openrouter":  # Add OpenRouter provider
            ...
            try:
                ...
                raise ValueError(f"Error initializing OpenRouter: {str(e)}")
        else:
    ...
    def _setup_tools(self):
        try:
            ...
            # Initialize Brave Search with enhanced error handling
            brave_search = None
            if brave_search_tool_available:
                if self.brave_api_key:
                    try:
                        ...
                        logging.debug("Brave Search initialization details:", exc_info=True)
                else:
                    logging.warning("Brave API key not set. Brave search tool will be unavailable.")
                    print("Note: Brave API key not set. Brave search tool will be unavailable.")
            else:
                logging.warning("BraveSearchRun is not available. Ensure langchain-community is installed with Brave Search support.")
    ...
    def _web_search_wrapper(self, func):
        def wrapper(query):
            ...
            try:
                self.web_limiter.try_acquire(self.WEB_SEARCH_CONTEXT)
                result = func(query)
                
                # Try crawl4ai as fallback if primary search fails or returns no results
                if (not result or "Error" in result) and crawl4ai_available:
                    try:
                        crawl4ai_fallback = Crawl4AIFallback()
                        fallback_result = crawl4ai_fallback.run(query)
                        if (fallback_result and "Error" not in fallback_result):
                            result = fallback_result

                    except Exception as e:
                        # Log fallback error but continue with original result
                        print(f"Crawl4AI fallback error: {str(e)}")
    ...
    with gr.Blocks(title="Advanced Research Assistant V4") as iface:
        ...
        with gr.Tab("Settings"):
            llm_provider_selection = gr.Dropdown(
                ["anthropic", "openai", "google", "vertexai", "openrouter"],  # Add openrouter option
                value="anthropic", 
